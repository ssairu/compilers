...
class Position:
    ...
    def is_newline(self):
        if self.index == len(self._text):
            return True
        if self._text[self.index] == "\r" and self.index + 1 < len(self._text):
...
class Scanner:
    ...
    def comments(self):
        return [
            [c, self.program[c.starting.index : c.following.index]]
    ...
    def next_token(self):
        while self._cur.get_char():
            ...
            self._compiler.add_message(
                True, start_pos, f"Unexpected character: {current_char}"
            )
            self._cur = self._cur.next()
    ...
    def _skip_comment(self, start_pos: Position):
        ...
        while self._cur.get_char():
            if (
                self._cur.get_char() == "*"
                and self._cur.next().get_char() == ")"
            ):
                end_pos = self._cur
                self._cur = self._cur.next().next()  # Пропускаем *)
                self._comments.append(
                    Fragment(start_pos.next().next(), end_pos)
                )
                return
            self._cur = self._cur.next()
    ...
    def _read_identifier(self, start_pos: Position):
        ...
        if self._cur.get_char() != ")":
            self._compiler.add_message(
                True, start_pos, "Expected closing parenthesis for identifier"
            )
            return None
        self._cur = self._cur.next()  # Пропускаем )
        end_pos = self._cur
    ...
    def _read_keyword(self, start_pos: Position):
        ...
        self._compiler.add_message(
            True, start_pos, f"Unknown keyword: {lexeme}"
        )
        return None
...
class Node:
    ...
    def __str__(self):
        return (
            self.term
            + "   "
            + str(self.token.coords if self.token else "none")
            + " : "
            + str(self.children)
            + "  ___  "
            + str(self.attr)
...
class ParserRules:
        ...
        while node_stack:
            ...
            if X in terminals + ["$"]:
                if X == a:
                    ...
                    self.add_message(
                        True, token.coords.starting, f"Expected {X}, got {a}"
                    )
                    return None
            else:
                key = (X, a)
                if key in self.table:
                    ...
                    self.add_message(
                        True, token.coords.starting, f"No rule for {X} on {a}"
                    )
                    return None
    ...
    def to_graphviz(self, root: Node) -> str:
        ...
        def _traverse(
            node: Node, dot_lines: list, node_ids: dict, counter: list
        ):
...
class TableGenerator:
    ...
    def get_rules(self, root: Node):
        ...
        def traverse(node: Node):
            ...
            # Non-terminal nodes
                ...
                # SP ::= id SP | ε
                node.attr = (
                    node.children[0].attr + node.children[1].attr
                    if node.children
                    else []
                )
                ...
                if node.children[1].attr[0] in self.terminals:
                    self.parser.compiler.add_message(
                        True,
                        node.children[1].attr[0].coords.starting,
                        f"Repeated terminal: (
                            {self.parser.compiler.get_name(node.children[1].attr[0].code)})",
                    )
        ...
        if not self.startingNT:
            self.parser.compiler.add_message(
                True,
                self.parser.scanner.next_token().coords.starting,
                f"axiom is not set",
            )
        ...
        while not (self.first == new_first):
            ...
            for NT, rule in self.rules:
                if not rule:
                    ...
                    first_k = self.FirstK_concatStringSets(
                        [new_first[symbol] for symbol in rule]
                    )
                ...
                for i, symbol in enumerate(rule):
                    if symbol in self.allNTs:
                        follow_set = self.FirstK_concatStringSets(
                            [self.first[s] for s in rule[i + 1 :]]
                            + [new_follow[NT]]
                        )
    ...
    def createParseTable(self, grammar, name):
        ...
        for NT, rule in self.rules:
            for x in self.FirstK_concatStringSets(
                [self.first[s] for s in rule] + [self.follow[NT]]
            ):
        ...
        with open(name, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"axiom = '{self.compiler.get_name(self.startingNT.code)}'\n\n"
            )

            f.write("# Auto-generated parse terms\n")
            f.write(
                f"terminals = {[self.compiler.get_name(x.code) for x in self.terminals]}\n\n"
            )
                        ...
                        + ", ".join(
                            [
                                str(
                                    list(
                                        [
                                            self.compiler.get_name(term.code)
                                            for term in rule
                                        ]
                                    )
                                )
...
class Node:
    ...
    def __str__(self):
        er = str(self.token.coords if self.token else "none")
        return self.term + "   " + er + " : " + str(self.children) + "  ___  " + str(self.attr)
...
digraph ParseTree {
    ...
    node_4 [label="T 1 (1)"];
    node_1 -> node_4;
 { rank=same; node_2 -> node_4 [style=invis] }
    node_0 -> node_1;
    node_5 [label="E 1 (359)"];
    node_6 [label="plus sign"];
    node_5 -> node_6;
    node_7 [label="T (332)"];
    node_8 [label="F (2)"];
    node_9 [label="n = 2 (2)"];
    node_8 -> node_9;
    node_7 -> node_8;
    node_10 [label="T 1 (166)"];
    node_11 [label="star"];
    node_10 -> node_11;
    node_12 [label="F (166)"];
    node_13 [label="left paren"];
    node_12 -> node_13;
    node_14 [label="E (166)"];
    node_15 [label="T (32)"];
    node_16 [label="F (32)"];
    node_17 [label="n = 32 (32)"];
    node_16 -> node_17;
    node_15 -> node_16;
    node_18 [label="T 1 (1)"];
    node_15 -> node_18;
 { rank=same; node_16 -> node_18 [style=invis] }
    node_14 -> node_15;
    node_19 [label="E 1 (134)"];
    node_20 [label="plus sign"];
    node_19 -> node_20;
    node_21 [label="T (134)"];
    node_22 [label="F (134)"];
    node_23 [label="n = 134 (134)"];
    node_22 -> node_23;
    node_21 -> node_22;
    node_24 [label="T 1 (1)"];
    node_21 -> node_24;
 { rank=same; node_22 -> node_24 [style=invis] }
    node_19 -> node_21;
    node_25 [label="E 1 (0)"];
    node_19 -> node_25;
 { rank=same; node_20 -> node_21 -> node_25 [style=invis] }
    node_14 -> node_19;
 { rank=same; node_15 -> node_19 [style=invis] }
    node_12 -> node_14;
    node_26 [label="right paren"];
    node_12 -> node_26;
 { rank=same; node_13 -> node_14 -> node_26 [style=invis] }
    node_10 -> node_12;
    node_27 [label="T 1 (1)"];
    node_10 -> node_27;
 { rank=same; node_11 -> node_12 -> node_27 [style=invis] }
    node_7 -> node_10;
 { rank=same; node_8 -> node_10 [style=invis] }
    node_5 -> node_7;
    node_28 [label="E 1 (27)"];
    node_29 [label="plus sign"];
    node_28 -> node_29;
    node_30 [label="T (7)"];
    node_31 [label="F (7)"];
    node_32 [label="n = 7 (7)"];
    ...
    node_33 [label="T 1 (1)"];
    node_30 -> node_33;
 { rank=same; node_31 -> node_33 [style=invis] }
    node_28 -> node_30;
    node_34 [label="E 1 (20)"];
    node_35 [label="plus sign"];
    node_34 -> node_35;
    node_36 [label="T (20)"];
    node_37 [label="F (20)"];
    node_38 [label="n = 20 (20)"];
    node_37 -> node_38;
    node_36 -> node_37;
    node_39 [label="T 1 (1)"];
    node_36 -> node_39;
 { rank=same; node_37 -> node_39 [style=invis] }
    node_34 -> node_36;
    node_40 [label="E 1 (0)"];
    node_34 -> node_40;
 { rank=same; node_35 -> node_36 -> node_40 [style=invis] }
    node_28 -> node_34;
 { rank=same; node_29 -> node_30 -> node_34 [style=invis] }
    node_5 -> node_28;
 { rank=same; node_6 -> node_7 -> node_28 [style=invis] }
    node_0 -> node_5;
 { rank=same; node_1 -> node_5 [style=invis] }
}
...
# Вывод
В процессе выполнения данной лабораторной работы я освоил методы 
построения LL(1) таблиц, научился делать обход дерева разбора и выполнения
семантического анализа для различных целей.